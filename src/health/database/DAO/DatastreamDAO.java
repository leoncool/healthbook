...
public class DatastreamDAO extends BaseDAO {
    ...
    public Datastream getDatastream(String StreamID, boolean fetchDataUnits, boolean fetchDatablocks) {
...
//        stream = (Datastream) session.get(Datastream.class, StreamID);
        ...
        stream = (Datastream) criteria.add(Restrictions.idEq(StreamID)).uniqueResult();
        if (session.isOpen()) {
            session.close();
        }
        return stream;
    ...
    public List<Datastream> getDatastreamList(Integer SubjectID, boolean fetchDataUnits, boolean fetchDatablocks) {
        Session session = HibernateUtil.beginTransaction();
        System.out.println(SubjectID);
        Criteria criteria = session.createCriteria(Datastream.class)
                .add(Restrictions.eq("subId", SubjectID));
//        if (fetchDataUnits) {
//            criteria.setFetchMode("datastreamUnitsList", FetchMode.JOIN);            
//        }
//        if (fetchDatablocks) {
...
//        }
    ...
    public List<DatastreamUnits> getDatastreamUnits(String datastreamID) {
        Session session = HibernateUtil.beginTransaction();
        Criteria criteria = session.createCriteria(DatastreamUnits.class)
                .add(Restrictions.eq("streamID", datastreamID));
    ...
    public Datastream createDatastream(Datastream streamObject, List<DatastreamUnits> unitList) {
        try {
            Session session = HibernateUtil.beginTransaction();
    ...
    public DatastreamBlocks CreateDatastreamBlock(Datastream streamID, String blockName, String blockDesc) {
        try {
            Session session = HibernateUtil.beginTransaction();
    ...
    public List<DatastreamBlocks> getDatastreamBlockList(String streamID) {
        ...
        criteria.add(Restrictions.eq("streamID", streamID)).addOrder(Order.asc("created"));;
        try {
    ...
    public DatastreamBlocks getDatastreamBlock(String blockID) {
        Session session = HibernateUtil.beginTransaction();
        DatastreamBlocks object = (DatastreamBlocks) session.get(DatastreamBlocks.class, blockID);
        try {
            return object;
        } catch (Exception ex) {
            ex.printStackTrace();

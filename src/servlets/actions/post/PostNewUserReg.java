...
public class PostNewUserReg extends HttpServlet {
    ...
    public void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        Gson gson = new Gson();
        try {
            JsonUser juser = gson.fromJson(jutil.readJsonStrFromHttpRequest(request), JsonUser.class);
            if (juser.getLoginid() == null || juser.getPassword() == null || juser.getEmail() == null) {
            ...
            if (userdao.EmailExists(juser.getEmail()) == true) {
                ReturnParser.outputErrorException(response, AllConstants.ErrorDictionary.email_Exist, null, juser.getEmail());
                out.close();
                return;
            }
            if (userdao.getLogin(juser.getLoginid()) == null) {
                ...
                out.println(ReturnParser.returnValidResult(AllConstants.ValidDictionary.Valid));
...
//                new EmailThread(emailSubject, emailBody, user.getEmail(), fromEmailAddress).start();
                ...
                out.println(ReturnParser.returnErrorException(AllConstants.ErrorDictionary.LoginID_Exist, null, juser.getLoginid()));
                out.close();
                return;
            }
